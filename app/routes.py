...
def home():
    ...
    goal_graph,result_graph,poss_graph = home_graph_match('match_data.csv')
    return render_template('home.html',graph=graph,goal_graph=goal_graph,result_graph=result_graph,poss_graph=poss_graph)

@main.route('/2425season')
def home_2425season():
    graph = home_graph('player_data2425.csv')
    goal_graph,result_graph,poss_graph = home_graph_match('match_data2425.csv')
    return render_template('home.html',graph=graph,goal_graph=goal_graph,result_graph=result_graph,poss_graph=poss_graph)
